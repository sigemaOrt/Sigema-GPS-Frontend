...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  private suscribirseAEstadoTrabajo() {
    this.trabajoSubscription = this.trabajoService.trabajoActivo$.subscribe(activo => {
  ...
  cargarEquipos() {
    ...
    this.equipoService.obtenerEquipos().pipe(
      finalize(() => (this.estaActualizando = false))
    ).subscribe({
      next: (data: Equipo[]) => {
        this.equipos = data;
      ...
      error: () => {
        this.mostrarMensaje('Error al cargar los equipos. Intente de nuevo más tarde.', 'error');
      }
    });
  }
  ...
  cargarEstadoUsoEquipos() {
    ...
    const estadoObservables: Observable<boolean>[] = this.equipos.map(equipo =>
      this.trabajoService.getEstaEnUso(equipo.id).pipe(catchError(() => of(false)))
    );
    ...
    forkJoin(estadoObservables).subscribe({
      ...
      error: () => this.mostrarMensaje('No se pudo verificar el estado de los equipos.', 'error')
    });
  }
  ...
  getTextoBoton(equipo: Equipo): string {
    ...
    if (this.hayTrabajoActivo && this.equipoEnTrabajoId !== equipo.id) return 'Trabajo en Progreso';
  ...
  toggleEstado(equipoId: number, estaIniciadoActual: boolean) {
    if (!estaIniciadoActual && !this.puedeIniciarTrabajo(equipoId)) {
      this.mostrarMensaje('No se puede iniciar un nuevo trabajo. Ya hay un trabajo en progreso.', 'warning');
      return;
    }
    estaIniciadoActual ? this.finalizarTrabajo(equipoId) : this.iniciarTrabajo(equipoId);
  }
    ...
    navigator.geolocation.getCurrentPosition(
      pos => {
        ...
        this.trabajoService.iniciarTrabajo(equipoId, lat, lon).subscribe({
          ...
          error: () => this.mostrarMensaje('Error al iniciar el trabajo.', 'error')
        });
      },
  ...
  private finalizarTrabajo(equipoId: number) {
    navigator.geolocation.getCurrentPosition(
      pos => {
        ...
        const lon = pos.coords.longitude;
        const equipo = this.equipos.find(e => e.id === equipoId);
        this.trabajoService.finalizarTrabajo(equipoId, lat, lon, equipo?.unidad?.emails ?? []).subscribe({
          next: () => {
            if (equipo) equipo.estaEnUso = false;
            this.mostrarMensaje('Trabajo finalizado exitosamente.', 'success');
          },
          error: () => this.mostrarMensaje('Error al finalizar el trabajo.', 'error')
        });
      },
  ...
  private manejarErrorGeolocation(error: GeolocationPositionError, accion: string) {
  ...
  private mostrarMensaje(msg: string, icon: 'success' | 'error' | 'warning' | 'info' = 'info') {
    Swal.fire({
      text: msg,
      icon: icon,
